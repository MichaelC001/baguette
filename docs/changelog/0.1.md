...
- **Input survives Xcode 27's Device Hub.** Device Hub attaches a HID
  ...
  [`docs/features/device-hub.md`](../features/device-hub/README.md).
...
- **Device twin: the host side of mirroring a physical iPhone.** A
  ...
  See [`docs/features/device-twin.md`](../features/device-twin/README.md).
...
- **Network conditioning — `baguette network set|clear|status`.** Makes a
  ...
  works the way the [virtual camera](../features/camera/README.md) and
  [motion](../features/motion/README.md) do. **Only apps launched after
  ...
  **zero** of the app's own requests over 100 seconds. Swizzling
  ...
  [`docs/features/network.md`](../features/network/README.md).
...
- **Install plugins from the browser — from bakeries you already trust.**
  ...
  **Trusting a new source is still not something a page can do** —
  ...
  [`docs/features/plugins.md`](../features/plugins/README.md).
...
- **Motion — `baguette motion start|set|stop`, and a walk that drives it.**
  ...
  So this works the way the [virtual camera](../features/camera/README.md) does:
  ...
  [`docs/features/motion.md`](../features/motion/README.md).
- **One output size for every capture — `--size appstore-6.9`, and the
  ...
  See [`docs/features/capture-size.md`](../features/capture-size/README.md).
...
- **Deep links — `baguette openurl <url>` and `baguette schemes`.** Opens a link
  ...
  See [`docs/features/deep-links.md`](../features/deep-links/README.md).
...
- **Companion screens rail.** A rail on the right edge of `/simulators/<udid>`
  ...
  [`docs/features/companion-screens.md`](../features/companion-screens/README.md).
...
- **Interface settings — appearance, contrast, text size.** The three
  ...
  capability. See [`docs/features/interface.md`](../features/interface/README.md).
...
- **Shake gesture.** `baguette shake --udid <UDID>`, `POST
  ...
  See [`docs/features/shake.md`](../features/shake/README.md).
...
- **3D "Interact" mode maps clicks and edge drags correctly at any camera
  ...
  [`docs/features/3d-rendering.md`](../features/3d-rendering/README.md).
...
- **Perspective, Retina-quality 3D rendering.** Live and one-shot 3D now share
  ...
  See [`docs/features/3d-rendering.md`](../features/3d-rendering/README.md).
...
- **Boot a device from its own tab.** Opening `/simulators/<udid>` on a
  ...
  [`docs/features/boot.md`](../features/boot/README.md).
...
### Added
- **Accessibility inspector overlay in the browser UI.** Hovering the live stream now highlights the AX node under the cursor with a translucent box + role/label tooltip; clicking locks the selection and exposes **Copy id** / **Copy JSON** / **Tap (cx, cy)** actions. Two surfaces share one inspector module: a sidebar checkbox card on `/simulators` (sidebar mode) and a toolbar icon next to the bezel-actionable toggle on `/simulators/<UDID>` (focus mode), with selection details surfacing in a glass-styled floating panel anchored top-right of the device column. Hit-testing runs client-side against a cached AX tree (mirroring `AXNode.hitTest` on the Swift side, so the browser overlay and the `describe-ui --x --y` CLI always pick the same element). The cache is refreshed on every fresh hover (mouseenter on the screen) and every click — no polling timer; idle pages cost nothing. Reuses the existing `/simulators/:udid/stream` WebSocket (sends `{"type":"describe_ui"}`, receives `{"type":"describe_ui_result","ok":true,"tree":…}`); no new endpoints, no extra connections. The "Tap" button forwards the centre of the locked frame as a canonical `{"type":"tap","x":…,"y":…,"width":…,"height":…}` envelope, so the inspector composes with every gesture path. See [`docs/features/ax-inspector.md`](../features/ax-inspector/README.md).

### Changed
- **Logs panel no longer stalls the page under CoreDuet-chatter floods.** Server-side `LogBatcher` (`Domain/Logs/LogBatcher.swift`) coalesces emitted lines into bounded batches that flush either at a 200-line size cap or after a 50 ms time window, replacing the per-line `{"type":"log","line":"…"}` text frames with one `{"type":"log","lines":["…","…"]}` envelope per ~20 frames/sec; clients still tolerate the old single-line shape during rolling upgrades. The browser-side `LogPanel` (`Resources/Web/sim-logs.js`) now renders incrementally — only newly arrived lines pay the regex-colourize cost on a `DocumentFragment`-driven append, instead of `innerHTML`-rebuilding the whole 1500-row buffer per frame; filter / clear / level / reveal trigger a one-shot full rebuild. An `IntersectionObserver` pauses rendering entirely when the panel is hidden (collapsed sidebar, off-screen sheet) and does one rebuild on reveal. WS frame rate is now bounded at ~20/sec regardless of log volume, and per-frame DOM cost is O(new lines) instead of O(buffer). See [`docs/features/logs.md`](../features/logs/README.md).

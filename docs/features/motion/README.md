...
Unlike [`location`](../location/README.md), **there is no `simctl` verb behind
...
[`CLHeading`](../location/README.md#course-is-not-heading) is) or lie convincingly
...
injects two dylibs (this and the [virtual camera](../camera/README.md#sharing-dyld_insert_libraries)).
...
- **The magnetometer** (`isMagnetometerAvailable`) — a magnetic-field
  ...
  [documented as impossible](../location/README.md#course-is-not-heading) in the
...
- **Private-layout dependency.** The fabrication relies on measured ivar
  ...
  internals — the same one the [virtual camera](../camera/README.md) makes.

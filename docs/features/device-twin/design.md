---
description: Design of the physical-iPhone twin — three severable pipes (pixels, sensors, input), why ReplayKit, CoreMotion-in-the-extension and an XCUITest runner, the gyro-twin render principles, and the passthrough/lazy-decode performance model.
---
...
`Infrastructure/` like every other context, plus a phone-side Tuist
sub-project, `Companion/DeviceTwin/`, beside `Injected/`.
...
rides `?format=` exactly as it does on `/simulators/:udid/stream`. The
route list itself lives in [README.md](README.md#http--websocket).
